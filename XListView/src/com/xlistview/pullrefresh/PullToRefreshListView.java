...
import com.xlistview.res.AttrRes;
import com.xlistview.res.ResUtil;
import com.xlistview.res.StyleableRes;
	...
	protected ListView createRefreshableView(Context context, AttributeSet attrs) {
		...
		if (attrs == null)
			...
			TypedArray ta = context.obtainStyledAttributes(attrs,
					new int[] { mResUtil.getIdFromAttr(AttrRes.type) });
			int type = ta.getInt(
					mResUtil.getIdFromStyleable(StyleableRes.ItemType_type), 0);
			if (type == 0)
				listView = new ListView(context);
			else
				listView = new SlideListView(context);
		}
		mListView = listView;
