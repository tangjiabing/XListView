...
import com.xlistview.pullrefresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mHeaderLayout.showContainer();
		mFooterLayout.showContainer();
	}
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mRefreshableView instanceof SlideListView) {
				SlideListView listView = (SlideListView) mRefreshableView;
				if (listView.isScrolling() || listView.isScrollOut()
						|| listView.isUpWhenScrollIn() == false) {
					mIsHandledTouchEvent = false;
					break;
				}
			}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mRefreshableView instanceof SlideListView) {
				SlideListView listView = (SlideListView) mRefreshableView;
				if (listView.isActionCancel())
					listView.actionMove(ev);
			}
			...
			if (mIsHandledTouchEvent) {
				...
				if (isReadyForPullDown()) {
					...
					if (mPullRefreshEnabled
						...
						if (mRefreshableView instanceof SlideListView)
							((SlideListView) mRefreshableView)
	...
	public void onPullDownRefreshComplete() {
		if (isPullRefreshing()) {
			...
			if (mRefreshableView instanceof SlideListView)
				((SlideListView) mRefreshableView).setIsRefreshing(false);
		}
	}
